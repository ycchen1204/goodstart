...
1. 資料庫與檔案儲存：GoodStart 使用 Supabase 專案 `lwzptiqgtfxvkdyasjcm`。請以專案管理者身分於 Supabase SQL Editor 套用 `supabase/migrations/0001_goodstart_initial.sql`；它會建立私密資料表及不公開的 `meal-images` 儲存空間。於 Vercel 設定 `NEXT_PUBLIC_SUPABASE_URL`、`NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` 與僅供伺服器使用的 `SUPABASE_SECRET_KEY`。
